...
|-----------|--------|-----------|
| Entries (entry) control service | ✅ | `POST /Entries/registrar` — Entries.API |
| Daily consolidated service | ✅ | `GET /Consolidations/saldo` — Consolidation.API |
...
|-----------|--------|---------------|
| Entries does not crash if Consolidation crashes | ✅ | Services decoupled via RabbitMQ; Outbox Pattern guarantees delivery |
| 50 req/s on Consolidation with ≤5% loss | ✅ | Redis cache (99%+ hit rate); NBomber Stress Test validates SLA |
...
|-----------|-------------|
| **Entries.API** | `POST /Entries/registrar` — Wolverine handlers, Vault, JWT, Idempotency |
| **Entries.Core** | CQRS via Wolverine, FluentValidation, EF Core 10, manual Outbox |
| **Consolidation.API** | `GET /Consolidations/saldo?data-lancamento=yyyy-MM-dd` via Redis |
| **Consolidation.Core** | Command + Query handlers via Wolverine |
| **Consolidation.Worker** | RabbitMQ consumer → updates Redis with distributed lock |
...
| **Kong rate-limiting** | 300 req/min (Entries) · 55 req/s (Consolidation) |
| **Kong proxy-cache** | Cache GET Consolidation for 30s |
...
| **Unit tests** | 83 tests: 48 (Entries) + 35 (Consolidation), zero failures |
| **Functional tests (business rules)** | 12 tests in `ConsolidationRegrasDenegocioTests` covering credits, debits, fallback, retroactive, and monetary precision |
| **Stress Tests** | NBomber 5.5.0 — 50 req/s (Consolidation) + 10 req/s (Entries) in parallel; auto-fetch JWT from Keycloak; HTML + Markdown reports |
...
|------|--------------|
| `totalCreditos`/`totalDebitos` in consolidated balance | `SaldoConsolidation` model redesign (currently only stores `Saldo` + `UltimaAtualizacao`) |
...
### 1. Entries Extract (`GET /Entries/extrato`)
Paginated query endpoint for Entries by period, with filters by type and date ordering. Would require separate `ReadModel` in Entries module.
...
### 4. Migration of Consolidation to autonomous microservice
`Consolidation.Worker` and `Consolidation.API` share infrastructure today. In real production, each would be an independent deployment with its own Vault, Redis, and scalability configuration.
...
Configure Kong's `response-ratelimiting` plugin with fallback to cached response when Consolidation is degraded, reducing errors perceived by client.
