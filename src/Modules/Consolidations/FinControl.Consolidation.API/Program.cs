...
using FinControl.Consolidation.API.Configuration;
...
try
{
    builder.AddFinControlSerilog("fincontrol-Consolidation");
}
catch (Exception ex) when (builder.Environment.IsDevelopment())
{
...
try
{
    builder.AddFinControlObservability("fincontrol-Consolidation");
}
catch (Exception ex) when (builder.Environment.IsDevelopment())
{
...
app.UseSubscriptionKeyValidation(VaultKeys.KongConsolidationSubscriptionKey);
